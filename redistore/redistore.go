...
func dial(network, address, password string,useTLS bool,) (redis.Conn, error) {
	//,redis.DialUseTLS(true),redis.DialTLSConfig(&tls.Config{})
	c, err := redis.Dial(network, address,redis.DialUseTLS(useTLS),redis.DialTLSConfig(&tls.Config{}))
	if err != nil {
		return nil, err
	}
	if password != "" {
...
func dialWithDB(network string, address string, password string, DB string, useTLS bool) (redis.Conn, error) {
