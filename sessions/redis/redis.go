...
import (
	...
	"github.com/gin-hy/mysessions/redistore"
	"github.com/gin-hy/mysessions/sessions"
)
...
func NewStore(size int, network, address, password string, useTLS bool, keyPairs ...[]byte) (Store, error) {
	s, err := redistore.NewRediStore(size, network, address, password, useTLS, keyPairs...)
	if err != nil {
		return nil, err
	}
	return &store{s}, nil
...
func NewStoreWithDB(size int, network, address, password, DB string, useTLS bool, keyPairs ...[]byte) (Store, error) {
	s, err := redistore.NewRediStoreWithDB(size, network, address, password, DB, useTLS, keyPairs...)
	if err != nil {
		return nil, err
	}
	return &store{s}, nil
